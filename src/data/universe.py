...
#
# Curated "Invisible Champion" micro/small-cap pools — companies with structural
# monopolies or duopolies over physical components that trillion-dollar industries
# cannot function without.  Organised by bottleneck theme.
#
# Sector tags show which booming ETF sectors make each theme most relevant.
# When XLK (Technology) or XLB (Materials) boom, ALL themes become candidates;
# sector-specific tags allow the Serenity layer to rank themes by relevance.

SUPPLY_CHAIN_THEMES: Dict[str, dict] = {
    "cpo_silicon_photonics": {
        "name":    "Co-Packaged Optics / Silicon Photonics",
        "thesis":  "Copper wiring hitting bandwidth & thermal wall at 400G+. "
                   "CPO is the structural replacement — pure-play component makers "
                   "hold monopoly positions in this <$2 B market.",
        "sectors": ["XLK", "XLC"],           # tech + comms infra boom triggers this
        "stocks":  ["COHR", "LITE", "FN", "VIAV", "IPGP", "AAOI", "IIVI"],
    },
    "sic_power_substrate": {
        "name":    "Silicon Carbide Wafer & Power Substrate",
        "thesis":  "AI server GPU clusters + EV inverters hit physical power-density "
                   "limits of silicon. SiC substrates are the irreplaceable bottleneck; "
                   "Wolfspeed controls ~40 % of global SiC wafer supply.",
        "sectors": ["XLK", "XLY", "XLI"],   # tech, autos, industrials
        "stocks":  ["WOLF", "ON", "AEIS", "VICR", "MPWR", "NOVT"],
    },
    "wafer_substrate_monopolies": {
        "name":    "Semiconductor Wafer & Substrate Monopolies",
        "thesis":  "Sub-$500 M niches with >80 % market share. "
                   "Advanced photomasks, CMP pads, and probe cards are "
                   "bottleneck components that leading-edge fabs cannot substitute.",
        "sectors": ["XLK", "XLB"],           # semis + materials
        "stocks":  ["ENTG", "ONTO", "UCTT", "ICHR", "KLIC", "FORM", "PLAB", "COHU", "MKSI"],
    },
    "specialty_gases_chemistry": {
        "name":    "Specialty Process Gases & Chemical Precursors",
        "thesis":  "Ultra-pure NF3, WF6, and high-k precursors are structural "
                   "monopolies — a handful of suppliers control 100 % of what "
                   "leading-edge EUV fabs can buy.",
        "sectors": ["XLB", "XLK"],
        "stocks":  ["APD", "LIN", "CBT", "ENTG", "AZTA", "IOSP"],
    },
    "advanced_packaging": {
        "name":    "Advanced Semiconductor Packaging & Chiplets",
        "thesis":  "Moore's Law cost-scaling stalled. Chiplet integration via "
                   "advanced packaging (CoWoS, HBM stacking, Fan-Out) is the "
                   "next primary scaling vector — equipment makers are the picks-and-shovels.",
        "sectors": ["XLK"],
        "stocks":  ["AMKR", "KLIC", "COHU", "UCTT", "FN", "ONTO", "ASX"],
    },
    "ai_thermal_management": {
        "name":    "AI Data Center Thermal & Power Delivery",
        "thesis":  "H100/B200 GPU racks dissipate 50–100 kW per rack — "
                   "legacy air cooling hitting physical ceiling. Liquid cooling "
                   "and advanced power conversion are the new bottleneck.",
        "sectors": ["XLK", "XLI"],
        "stocks":  ["VICR", "AEIS", "GTLS", "BELFB", "NOVT", "CAMT"],
    },
    "rare_earth_magnets": {
        "name":    "Rare Earth & Critical Mineral Supply Chain",
        "thesis":  "EV motors, wind turbines, and military guidance systems "
                   "all depend on neodymium-iron-boron magnets. "
                   "MP Materials controls the only active US rare earth mine.",
        "sectors": ["XLB", "XLY", "XLI"],
        "stocks":  ["MP", "USAS", "NEM", "FCX", "ALTM"],
    },
}

# Flat de-duplicated list of all Serenity stocks for bar pre-fetching
_ALL_SERENITY_STOCKS: List[str] = list(dict.fromkeys(
    s for theme in SUPPLY_CHAIN_THEMES.values() for s in theme["stocks"]
))
...
class DynamicUniverse:
    def __init__(self, cfg: dict):
        ...
        # Daily Serenity cache — supply chain theme picks
        self._serenity_picks: List[str] = []
        self._serenity_date: Optional[date] = None
        self.serenity_stocks: int = int(cfg.get("serenity_stocks", 3))  # slots for bottleneck names
        self.use_serenity: bool = bool(cfg.get("serenity_screen", True))
    ...
    def _serenity_gemini_call(
        self,
        active_sectors: List[str],
        sector_scores: Dict[str, float],
    ) -> List[str]:
        """One daily Gemini call framed around the Serenity thesis.

        Tells Gemini which sectors are booming today and asks it to rank the
        supply-chain bottleneck themes by conviction.  Returns an ordered list
        of theme keys (highest conviction first).  Cached per day.
        """
        today = date.today()
        if self._serenity_date == today and self._serenity_picks:
            log.info("Serenity Gemini: using today's cached theme picks %s.", self._serenity_picks)
            return self._serenity_picks

        if not self._gemini_key:
            log.debug("Serenity Gemini call skipped — no GEMINI_API_KEY.")
            return []

        # Describe booming sectors
        sector_desc = ", ".join(
            f"{SECTORS.get(e, e)} ({e}, score={sector_scores.get(e, 0):+.3f})"
            for e in active_sectors
        )

        # Describe available themes
        theme_lines = []
        for key, meta in SUPPLY_CHAIN_THEMES.items():
            relevant = any(s in active_sectors for s in meta["sectors"])
            stocks_str = ", ".join(meta["stocks"][:5])
            theme_lines.append(
                f'  "{key}": {meta["name"]}\n'
                f'    Thesis: {meta["thesis"]}\n'
                f'    Key stocks: {stocks_str}\n'
                f'    Relevant to today\'s boom: {"YES" if relevant else "maybe"}'
            )

        prompt = (
            f"Today is {today.strftime('%A, %B %d %Y')}.\n\n"
            f"Today's booming market sectors (quantitatively confirmed): {sector_desc}\n\n"
            "You are Serenity, an expert in supply chain bottleneck investing.\n"
            "Your edge: identifying micro-cap and small-cap 'Invisible Champions' — "
            "companies with structural monopolies over physical components that "
            "trillion-dollar industries cannot function without.\n\n"
            "Available supply chain themes to evaluate today:\n\n"
            + "\n".join(theme_lines)
            + "\n\n"
            "Based on:\n"
            "  • Today's booming sectors and which themes they directly feed\n"
            "  • Current macro catalysts (AI capex, EV ramp, fab buildout, reshoring)\n"
            "  • Any forced liquidations or quant scanner mismatch setups visible in "
            "    micro-cap supply chain names\n"
            "  • Physical scaling constraints that create irreversible demand\n\n"
            f"Rank the TOP 2-3 themes by today's conviction. "
            "For each theme pick the 2 highest-conviction individual stocks.\n\n"
            "Respond with ONLY valid JSON — no markdown fences:\n"
            '{"themes": [{"key": "wafer_substrate_monopolies", "stocks": ["ENTG", "ONTO"], '
            '"reasoning": "one sentence"}, ...], "macro_note": "one sentence"}'
        )
        ...
        try:
            ...
            picks: List[str] = []
            for theme_entry in result.get("themes", []):
                key = theme_entry.get("key", "")
                if key not in SUPPLY_CHAIN_THEMES:
                    continue
                valid_pool = SUPPLY_CHAIN_THEMES[key]["stocks"]
                for sym in theme_entry.get("stocks", []):
                    if sym in valid_pool and sym not in picks:
                        picks.append(sym)
                log.info(
                    "Serenity theme [%s] → %s | %s",
                    SUPPLY_CHAIN_THEMES[key]["name"],
                    theme_entry.get("stocks", []),
                    theme_entry.get("reasoning", ""),
                )
            macro_note = result.get("macro_note", "")
            if macro_note:
                log.info("Serenity macro note: %s", macro_note)

            if picks:
                self._serenity_picks = picks
                self._serenity_date = today
            return picks

        except Exception as exc:
            log.warning("Serenity Gemini call failed: %s", exc)
            return []

    def _serenity_bottleneck_screen(
        self,
        active_sectors: List[str],
        sector_scores: Dict[str, float],
        stock_bars: pd.DataFrame,
    ) -> List[str]:
        """Serenity Supply Chain Bottleneck & Hardware Arbitrage screen.

        Three-step process run once per day:
          1. Gemini identifies highest-conviction themes for today's booming sectors.
          2. Quantitative bottleneck signals are computed for each theme's stocks:
               • Volume surge   — non-fundamental forced liquidation / reversal
               • Low-ATR spike  — quant scanner mismatch (algorithm-dropped micro-cap)
               • Momentum divergence vs sector — alpha opportunity vs passive flow
          3. Top N stocks are returned for injection into the daily pool.

        Falls back to a quant-only ranking if Gemini is unavailable.
        """
        if not self.use_serenity:
            return []

        # Step 1 — Gemini theme selection (daily cached)
        gemini_stocks = self._serenity_gemini_call(active_sectors, sector_scores)

        # Step 2 — Quantitative bottleneck scoring on ALL Serenity stocks
        # (so we can re-rank even when Gemini picks differ from top quant scores)
        quant_scores: Dict[str, float] = {}

        def _score_sym(sym: str) -> float:
            try:
                if stock_bars.empty or not isinstance(stock_bars.index, pd.MultiIndex):
                    return 0.0
                if sym not in stock_bars.index.get_level_values(0):
                    return 0.0
                df = stock_bars.loc[sym]
                ...
                # Volume surge: today vs 20-day mean (forced-liquidation reversal signal)
                vol_mean  = float(volume.tail(20).mean()) or 1.0
                vol_surge = float(volume.iloc[-1]) / vol_mean
                # A 2–4× surge is the sweet spot: meaningful but not panic
                vol_score = float(pd.Series([vol_surge]).clip(0.5, 4.0).map(
                    lambda v: (v - 0.5) / 3.5
                ).iloc[0])

                # ATR spike: last-bar ATR vs 10-day avg ATR
                # Quant scanner mismatch shows as sudden ATR spike on low-liquidity name
                n = min(11, len(df))
                hi = high.tail(n).values
                lo = low.tail(n).values
                cp = close.tail(n + 1).iloc[:-1].values
                min_len = min(len(hi), len(lo), len(cp))
                tr = pd.DataFrame({
                    "hl": hi[:min_len] - lo[:min_len],
                    "hc": abs(hi[:min_len] - cp[:min_len]),
                    "lc": abs(lo[:min_len] - cp[:min_len]),
                }).max(axis=1)
                atr_today  = float(tr.iloc[-1])
                atr_avg    = float(tr.mean()) or 1e-9
                atr_ratio  = atr_today / atr_avg
                # Ratio in [1.5, 4] = scanner mismatch zone → score ∈ [0, 1]
                atr_score  = float(min(max((atr_ratio - 1.5) / 2.5, 0.0), 1.0))

                # Momentum divergence: stock 5-day return vs its sector's 5-day return
                # Positive divergence = stock outrunning sector = alpha
                period = min(5, len(df) - 1)
                stock_mom = float(close.iloc[-1] / close.iloc[-period - 1] - 1) if period > 0 else 0.0

                # Find which theme this stock belongs to and its sector
                theme_sector_ret = 0.0
                for meta in SUPPLY_CHAIN_THEMES.values():
                    if sym in meta["stocks"]:
                        # Use the first matching sector ETF's score as a proxy
                        for etf in meta["sectors"]:
                            if etf in sector_scores:
                                theme_sector_ret = sector_scores[etf]
                                break
                        break
                mom_div   = stock_mom - theme_sector_ret
                mom_score = float(min(max(mom_div * 10 + 0.5, 0.0), 1.0))  # centre at 0

                # Sector relevance bonus: stock's theme sectors are in today's booming list
                relevance = 0.0
                for meta in SUPPLY_CHAIN_THEMES.values():
                    if sym in meta["stocks"]:
                        if any(s in active_sectors for s in meta["sectors"]):
                            relevance = 0.3
                        break
                ...
                log.debug("Serenity quant score failed for %s: %s", sym, exc)
                return 0.0

        for sym in _ALL_SERENITY_STOCKS:
            quant_scores[sym] = _score_sym(sym)

        quant_ranked = sorted(quant_scores, key=quant_scores.get, reverse=True)  # type: ignore[arg-type]

        # Step 3 — Blend: Gemini picks first (high conviction), backfill with quant
        blended: List[str] = []
        # Gemini picks that also have positive quant signal → front of list
        for sym in gemini_stocks:
            if sym in quant_scores and quant_scores[sym] > 0.1 and sym not in blended:
                blended.append(sym)
        # Fill remaining slots with pure quant picks
        for sym in quant_ranked:
            if len(blended) >= self.serenity_stocks:
                break
            if sym not in blended and quant_scores[sym] > 0.05:
                blended.append(sym)

        log.info(
            "Serenity bottleneck screen → %d stocks: %s",
            len(blended),
            [(s, f"q={quant_scores.get(s, 0):.2f}") for s in blended],
        )
        return blended
    ...
    def _select_sector_intelligence(self) -> List[str]:
        ...
        # Reserve serenity_stocks slots so the Serenity layer can inject its picks
        serenity_slots  = self.serenity_stocks if self.use_serenity else 0
        available_slots = max(1, self.max_tickers - serenity_slots)
        stocks_per_sector = max(1, available_slots // len(selected))
        chosen: List[str] = []
        for etf in selected:
            ranked = self._rank_stocks_in_sector(etf, stock_bars)
            chosen.extend(ranked[:stocks_per_sector])

        # ── 7. Serenity Supply Chain Bottleneck screen ─────────────────────
        # Fetches bars for all Serenity stocks, runs the Gemini + quant screen,
        # injects the top invisible-champion names into the pool.
        if self.use_serenity and serenity_slots > 0:
            serenity_symbols = [s for s in _ALL_SERENITY_STOCKS if s not in chosen]
            log.info(
                "Serenity: fetching 25-day bars for %d supply-chain bottleneck candidates …",
                len(serenity_symbols),
            )
            try:
                serenity_bars = self._fetch_daily_bars(serenity_symbols, lookback=25)
            except Exception as exc:
                log.warning("Serenity bar fetch failed (%s) — screen skipped.", exc)
                serenity_bars = pd.DataFrame()

            bottleneck_picks = self._serenity_bottleneck_screen(
                active_sectors=selected,
                sector_scores=scores,
                stock_bars=serenity_bars,
            )
            for sym in bottleneck_picks:
                if sym not in chosen:
                    chosen.append(sym)
                    log.info("Serenity injected: %s", sym)

        # ── 8. Add fallback tickers (SPY / QQQ) and cap ────────────────────
